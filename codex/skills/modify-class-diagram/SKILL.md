...
description: Update an existing UML class diagram (.drawio) so it better reflects a codebase's architecture. Use when the user provides an existing .drawio or draw.io XML file plus a target codebase and asks to update, refresh, sync, refactor, fix, audit, review, or diff the diagram against the code. Requires BOTH an existing class diagram and target codebase. Do NOT use for creating a class diagram from scratch, modifying sequence/state/system diagrams, or non-UML draw.io files.
---
...
Save the modified file back to the original path on disk when the user pointed at one; otherwise save it alongside the original as `<original-name>_updated.drawio`. Report the path in your reply.
